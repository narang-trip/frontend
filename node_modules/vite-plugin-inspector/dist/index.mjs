  ...
  try {
    if (process.platform === "darwin") {
      ...
      for (let i = 0; i < runningProcesses.length; i++) {
        ...
        const processName = path2.basename(processPath);
...
function printInstructions(fileName, errorMessage) {
  console.log();
  console.log("Could not open " + path2.basename(fileName) + " in the editor.");
  if (errorMessage) {
...
function middleware(req, res, next) {
  ...
  const cwd = process.cwd();
  try {
    open_editor_default(path3.join(cwd, filePath), parseInt(lineNumber));
    res.statusCode = 200;
    res.end("ok");
  } catch (err) {
