...
var vue_default = compileVue;

// src/core/transform.ts
function addHtmlAttr(filename, line, loc) {
  const cwd = process.cwd();
  const htmlTagReg1 = /<(\w+)(\s*>)/g;
  const htmlTagReg2 = /<(\w+)\s+([\w|:])/g;
  const replaceValue = `<$1 data-loc="${filename.replace(cwd, "")}:${loc}" $2`;
  const result = line.replace(htmlTagReg2, replaceValue).replace(htmlTagReg1, replaceValue);
  return result;
}
function generate(id, code, html) {
  if (!html) {
    html = {
      content: code,
      start: 0,
...
function transform(code, id, options = { serializePath: true }) {
  const extname = import_path.default.extname(id);
  let result;
  switch (extname) {
    case ".vue":
      result = generate(id, code, vue_default(code));
      break;
    case ".jsx":
    case ".tsx":
    case ".svelte":
  ...
  try {
    if (process.platform === "darwin") {
      ...
      for (let i = 0; i < runningProcesses.length; i++) {
        ...
        const processName = import_path2.default.basename(processPath);
...
function printInstructions(fileName, errorMessage) {
  console.log();
  console.log("Could not open " + import_path2.default.basename(fileName) + " in the editor.");
  if (errorMessage) {
...
function middleware(req, res, next) {
  ...
  const loc = queryParams.query.loc;
  if (!loc) {
    return next();
  }
  ...
  const cwd = process.cwd();
  try {
    open_editor_default(import_path3.default.join(cwd, filePath), parseInt(lineNumber));
    res.statusCode = 200;
    res.end("ok");
  } catch (err) {
...
// src/core/inject-script.ts
function inject_script_default(options) {
  return `
  ...
  return {
    ...
    transformIndexHtml: {
      ...
      transform() {
        return [{
          tag: "script",
          children: inject_script_default(normalizedOptions),
          injectTo: "head"
        }];
      }
    },
    configureServer({ middlewares }) {
      middlewares.use(normalizedOptions.route, middleware_default);
    },
    transform(code, id) {
      const { code: newCode } = transform_default(code, id);
      return newCode;
    }
  };
}
