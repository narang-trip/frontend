...
const userId = "조예진";
const stompEndpoint = `wss://i10a701.p.ssafy.io/api/message/chat/${userId}`
const ChatPage = () => {
  // useEffect(() => {
  ...
  useEffect(() => {
    ...
    const stompClient = new Client({
      ...
      onConnect: (frame) => {
        ...
        // 서버로부터 메시지를 받을 구독 설정
        stompClient.subscribe('/api/message/sub/chat/room/room1', message => {
          console.log(`Received: ${message.body}`);
        });

        // 서버로 메시지 전송
        stompClient.publish({
          destination: '/api/message/pub/chat/send',
          body: { chatRoomId : 1, senderId : "조예진", content : "이제 해방이다"},
        });
      },
    });
