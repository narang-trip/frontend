...
const Chat = ({ chatroomId }) => {
  ...
  return (
    <Fragment>
      ...
      <div className="pb-10 overflow-y-auto w-full pr-4 relative" ref={chatListRef}>
        {chats.map((chat) => {
          ...
          let messageClass = "text-sm rounded-lg border p-2 max-w-[80%] break-words";
          if (chat.userId === "관리자") {
            ...
            return (
              <div key={chat.chatId} className={messageBoxClass}>
                <div className="text-xs mr-1">{formattedDate}</div><div className={messageClass}>{chat.content}</div>
              </div>
            )
          } else {
            ...
            return (
              <div key={chat.chatId} className={messageBoxClass}>
                <div className={messageClass}>{chat.content}</div><div className="text-xs ml-1">{formattedDate}</div>
              </div>
            )
          }
